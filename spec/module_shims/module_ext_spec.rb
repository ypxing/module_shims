...
describe 'extension to Module' do
  context 'remove_existing_instance_methods' do
    ...
    it "can remove all methods from itself" do
      ...
      expect(@target.public_instance_methods(false)).to be_empty
      expect(@target.protected_instance_methods(false)).to be_empty
      expect(@target.private_instance_methods(false)).to be_empty
    end
